...
# Import CBAM model
from custom_models_cbam import SignLanguageRecognitionModel, I3DFeatureExtractor
...
def run(configs, mode='rgb', root='/ssd/Charades_v1_rgb', train_split='charades/charades.json', save_model='', pretrained_i3d_weights=None):
    ...
    # Optimizer - train CBAM and transformer
    lr = configs.init_lr
    weight_decay = configs.adam_weight_decay
    criterion = nn.CrossEntropyLoss(weight=class_weights_tensor.to(device))
    
    # Train both CBAM and transformer parameters
    trainable_params = list(model.module.cbam.parameters()) + list(model.module.transformer.parameters())
    ...
    for epoch in range(num_epochs):
        ...
        if val_epoch_accuracy > best_val_accuracy:
            ...
            # Save the best model
            checkpoint_path = os.path.join(checkpoint_dir, f"best_model_cbam_epoch{epoch}_acc{val_epoch_accuracy:.2f}.pth")
    ...
    if not early_stop:
        ...
        final_model_path = os.path.join(checkpoint_dir, 'final_model_cbam.pth')
...
if __name__ == '__main__':
    ...
    print("="*80)
    print("Sign Language Recognition with CBAM Attention")
    print("="*80)
